      ...
      <div className="bg-white rounded-2xl shadow-lg max-w-lg w-full relative">
        ...
        <div className="p-6">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            {item.name}
            {item.is_veg ? (
              <span className="w-3 h-3 bg-green-600 rounded-full" title="Vegetarian"></span>
            ) : (
              <span className="w-3 h-3 bg-red-600 rounded-full" title="Non-Vegetarian"></span>
            )}
          </h2>
          ...
          <div className="mt-6 flex items-center gap-4">
            <span className="text-xl font-bold text-teal-600">₹{finalPrice}</span>
            {item.discount > 0 && (
